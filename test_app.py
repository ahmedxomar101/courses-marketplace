...
def test_get_courses_filter_by_domain_and_sort_by_rating():
    response = client.get("/courses?domain=mathematics&sort_by=rating")
    assert response.status_code == 200
    courses = response.json()
    assert len(courses) > 0
    assert all([c['domain'][0] == 'mathematics' for c in courses])
...
def test_rate_chapter_not_exists():
    response = client.post("/courses/65d0bc32869153d3412ad583/990/rate", json={"rating": 1})
    assert response.status_code == 404
